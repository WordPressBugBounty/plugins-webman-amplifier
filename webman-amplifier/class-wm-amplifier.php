...
if ( ! class_exists( 'WM_Amplifier' ) ) {
	...
	class WM_Amplifier {
		...
		public function setup_actions() {
			...
			// Helper variables
				...
				$actions = array(
					...
					'register_icons'                => 'init|0',            // Register icon font
		 ...
		 * @access  public
		 */
		public function setup_action_links( $links ): array {

			// Processing

				// Icon font setup link
				if (
					(bool) apply_filters( 'wmhook_wmamp_enable_iconfont', true )
					&& ! wma_supports_subfeature( 'disable-fonticons' )
				) {

					$links[] =
						'<a href="' . esc_url( get_admin_url( null, 'themes.php?page=icon-font' ) ) . '">'
						. esc_html_x( 'Icon Font', 'Plugin action link.', 'webman-amplifier' )
						. '</a>';
				}

				// Themes link
				$links[] = '<a href="https://www.webmandesign.eu/"><strong>WebMan Design</strong></a>';
		...
		public function assets() {
			...
			// Variables
				...
				$display_isotope = apply_filters( 'wmhook_wmamp_notice_isotope_licence', true ) && ! wma_supports_subfeature( 'disable-isotope-notice' );
		...
		public function save_permalinks() {
			...
			if (
				isset( $_REQUEST['permalink_structure'] )
				&& isset( $_REQUEST['wmamp-permalinks'] )
				&& isset( $_REQUEST['_wpnonce'] )
				&& wp_verify_nonce( $_REQUEST['_wpnonce'], 'update-permalink' ) // phpcs:ignore
			) {

				$permalinks = array_filter( (array) $_REQUEST['wmamp-permalinks'] ); // phpcs:ignore
		...
		public function register_post_types() {
			...
			do_action( 'wmhook_wmamp_register_post_types' );
		...
		public function custom_taxonomies() {
			...
			// Variables
				...
				$taxonomies = (array) apply_filters( 'wmhook_wmamp_custom_taxonomies', array() );
		 ...
		 */
		public function post_types_in_feed( array $query ): array {
			...
			// Processing
				...
				if (
					...
					if (
						...
						&& apply_filters( 'wmhook_wmamp_post_types_in_feed_wm_modules', false )
					) {
					...
					if (
						...
						&& apply_filters( 'wmhook_wmamp_post_types_in_feed_wm_logos', false )
					) {
					...
					if (
						...
						&& apply_filters( 'wmhook_wmamp_post_types_in_feed_wm_projects', true )
					) {
					...
					if (
						...
						&& apply_filters( 'wmhook_wmamp_post_types_in_feed_wm_staff', false )
					) {
					...
					if (
						...
						&& apply_filters( 'wmhook_wmamp_post_types_in_feed_wm_testimonials', false )
					) {
			...
			// Output
				...
				return (array) apply_filters( 'wmhook_wmamp_post_types_in_feed_query', $query );
		...
		public function register_visual_editor_addons() {
			...
			// Processing
				...
				if (
					apply_filters( 'wmhook_wmamp_enable_visual_editor_addons', true )
		...
		public function register_icons() {
			...
			// Output
				...
				if (
					apply_filters( 'wmhook_wmamp_enable_iconfont', true )
					&& ! wma_supports_subfeature( 'disable-fonticons' )
				) {
		...
		public function admin_notices() {
			...
			// Variables
				...
				$capability = apply_filters( 'wmhook_wmamp_notice_capability', 'switch_themes' );
			...
			// Output
				...
				if ( $output ) {
					echo wp_kses_post( apply_filters( 'wmhook_wmamp_admin_notices_output', $output, $message ) );
				}
